...
/** Reads the agentcore.json spec an export wrote to the project. */
async function readSpec(project: Project) {
  return Bun.file(join(project.rootPath, "agentcore", "agentcore.json")).json();
}
...
describe("FsProjectManager.exportHarness", () => {
  test("emits a CodeZip runtime with no hooks, memory, or container files", async () => {
    const { manager: subject } = manager();
    const project = await projectWithHarness(subject);

    const result = await drainToResult(subject.exportHarness(project, exportInput()));

    for (const path of ["hooks", "memory", "Dockerfile", ".dockerignore"]) {
      expect(existsSync(join(result.agentPath, path))).toBe(false);
    }
  ...
  test.each([
    { provider: "open_ai", modelId: "gpt-4.1", credentialName: "OpenAiKey" },
    { provider: "gemini", modelId: "gemini-2.5-flash", credentialName: "GeminiKey" },
    { provider: "lite_llm", modelId: "gpt-4.1", credentialName: "LiteLlmKey" },
  ] as const)(
    "registers an API-key credential for a $provider model",
    async ({ provider, modelId, credentialName }) => {
      const { manager: subject } = manager();
      const project = await projectWithHarness(subject, {
        model: {
          provider,
          modelId,
          apiKeyArn: `arn:aws:bedrock-agentcore:us-east-1:111122223333:token-vault/default/apikeycredentialprovider/${credentialName}`,
        },
      });
      ...
      const spec = await readSpec(project);
      const credential = spec.credentials.find((c: { name: string }) => c.name === credentialName);
      expect(credential.authorizerType).toBe("ApiKeyCredentialProvider");
      expect(result.notes.map((note) => note.category)).toContain(
        "Model API key credential referenced",
      );
    },
  );

  test("notes when a keyless non-Bedrock LiteLLM model may need an API key", async () => {
    const { manager: subject } = manager();
    const project = await projectWithHarness(subject, {
      model: { provider: "lite_llm", modelId: "gpt-4.1" },
    });

    const result = await drainToResult(subject.exportHarness(project, exportInput()));

    expect(result.notes.map((note) => note.category)).toContain(
      "LiteLLM model may require an API key",
    );
  });
  ...
  test("notes that memory messagesCount is not portable to Strands", async () => {
    ...
    expect(existsSync(join(result.agentPath, "memory", "session.py"))).toBe(true);
    expect(result.notes.map((note) => note.category)).toContain(
  ...
  test("cleans up the agent dir and .env.local when the spec write fails", async () => {
    ...
    const { manager: subject } = manager({ json });
    const project = await projectWithHarness(subject, {
      tools: [
        {
          type: "remote_mcp",
    ...
    // The scaffolded .env.local survives, but the staged secret value is rolled back.
