...
interface PinState {
  // key and depth identify the current history entry and its position in the
...
// PinnedRegion hands every route a context whose RegionKey is the region a
// screen pinned (see usePinRegion) instead of the launch region. A pin belongs
// to the history entry that set it and to the entries pushed above it, so a
// detail page and what it opens fetch where the resource lives. Popping below
// that entry drops the pin, and a command menu drops every pin (see
// RouterScreen): from a menu the user starts over in the launch region.
function PinnedRegion({ ctx, core }: ScreenProps) {
  const { key } = useLocation();
  const navigationType = useNavigationType();
  const [state, setState] = useState<PinState>({ key, depth: 0, pins: [] });

  /**
   Track the history entry during render, so a screen that pins in its mount
   effect records the depth of its own entry.
  **/
  if (state.key !== key) {
    const depth =
      navigationType === "PUSH"
        ...
        : navigationType === "POP"
          ...
          : state.depth;
    const pins =
      navigationType === "POP" ? state.pins.filter((entry) => entry.depth <= depth) : state.pins;
    setState({ key, depth, pins });
  }

  const pin = useCallback((region: string | undefined) => {
    setState((current) => ({
      ...current,
      pins:
        region === undefined
          ? []
          : [
              ...current.pins.filter((entry) => entry.depth !== current.depth),
              { depth: current.depth, region },
            ],
    }));
  }, []);

  const pinned = state.pins.at(-1)?.region;
  ...
  return (
    <RegionPinContext.Provider value={pin}>
      <RouteTable ctx={routed} core={core} />
...
function RouteTable({ ctx, core }: ScreenProps) {
  return (
    <Routes>
      ...
      />
      <Route path="agentcore/memory/list" element={<MemoryListScreen ctx={ctx} core={core} />} />
      <Route
      ...
      />
      <Route path="agentcore/gateway/list" element={<GatewayListScreen ctx={ctx} core={core} />} />
      <Route
      ...
      />
      <Route path="agentcore/gateway/rule" element={<GatewayRuleScreen ctx={ctx} core={core} />} />
      <Route
      ...
      />
      <Route path="agentcore/memory/event" element={<MemoryEventScreen ctx={ctx} core={core} />} />
      <Route
      ...
      <Route
        ...
        element={<MemoryRecordListScreen ctx={ctx} core={core} />}
      />
      <Route path="agentcore/memory/actor" element={<MemoryActorScreen ctx={ctx} core={core} />} />
      <Route
